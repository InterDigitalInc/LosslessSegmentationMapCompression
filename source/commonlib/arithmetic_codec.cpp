...
void Arithmetic_Codec::encode(unsigned data, Adaptive_Data_Model &M, int * forbidden)
{
	...
	{
		if (M.data_symbols - forbidden[0] == 1) return;

		forbidden[forbidden[0] + 1] = 0;
		unsigned j = 1, k, sum = 0;
		unsigned data_code = data;
		unsigned count_rest = M.total_count;
		for (int i = 0; i < forbidden[0]; i++) count_rest -= M.symbol_count[forbidden[i + 1]];
		...
		for (k = 0; k < M.data_symbols; k++)
		{
			...
			if (k == data) data_code = k - (j - 1);
			M.distribution_forbidden[k - (j - 1)] = (scale * sum) >> (31 - DM__LengthShift);
			sum += M.symbol_count[k];
		}
...
float Arithmetic_Codec::getBits(unsigned data, Adaptive_Data_Model & M, int * forbidden)
{
	if (M.data_symbols == 1 || M.data_symbols - forbidden[0] == 1) return 0;
...
void Adaptive_Data_Model::copy(Adaptive_Data_Model & M, int* forbidden)
{
	...
	for (int i = 0; i < forbidden[0]; i++) total_count -= M.symbol_count[forbidden[i + 1]];
