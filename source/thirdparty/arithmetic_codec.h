...
public:
  ...
  void copy(int *count_list);
  void copy(Adaptive_Data_Model &M, int *forbidden);
  float get_bits(unsigned data);
  float get_bits(unsigned data, int forbidden);
  int get_count();

  unsigned *distribution, *distribution_forbidden, *symbol_count,
      *decoder_table;
  unsigned total_count, pre_count, update_cycle, symbols_until_update;
  unsigned data_symbols, last_symbol, table_size, table_shift;
...
// Class with both the arithmetic encoder && decoder.  All compressed data is
...
public:
  ...
  void noencode(unsigned data, Adaptive_Data_Model &);

  unsigned decode(Adaptive_Data_Model &);
  void nodecode(Adaptive_Data_Model &, int s);

  void encode(unsigned data, Adaptive_Data_Model &M, int forbidden);
  unsigned decode(Adaptive_Data_Model &M, int forbidden);

  void encode(unsigned data, Adaptive_Data_Model &M, int forbidden1,
              int forbidden2);
  unsigned decode(Adaptive_Data_Model &M, int forbidden1, int forbidden2);

  void encode(unsigned data, Adaptive_Data_Model &M, int *forbidden);
  float getBits(unsigned data, Adaptive_Data_Model &M, int *forbidden);
  unsigned decode(Adaptive_Data_Model &M, int *forbidden);
