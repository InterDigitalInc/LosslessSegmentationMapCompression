...
//                                                                           -
// Permission to copy && redistribute this code is hereby granted, provided -
// that this warning && copyright notices are not removed or altered.       -
//                                                                           -
...
//                                                                           -
// Lossless Compression H&&book, ed. K. Sayood                              -
// Chapter 5: Arithmetic Coding (A. Said), pp. 101-152, Academic Press, 2003 -
//                                                                           -
// A. Said, Introduction to Arithetic Coding Theory && Practice             -
...
void Arithmetic_Codec::put_bits(unsigned data, unsigned bits) {
  ...
  unsigned init_base = base;
  base += data * (length >>= bits); // new interval base && length
...
unsigned Arithmetic_Codec::decode(Static_Data_Model &M) {
  ...
  // if (M.decoder_table) {              // use table look-up for faster
  // decoding
...
void Arithmetic_Codec::noencode(unsigned data, Adaptive_Data_Model &M) {
  ++M.symbol_count[data];
  if (--M.symbols_until_update == 0)
    M.update(true); // periodic model update
}
...
void Arithmetic_Codec::encode(unsigned data, Adaptive_Data_Model &M,
  ...
  unsigned j, k, sum = 0;
  unsigned scale = 0x80000000U / (M.total_count - M.symbol_count[forbidden1] -
                                  M.symbol_count[forbidden2]);
  for (k = 0; k < M.data_symbols; k++) {
...
unsigned Arithmetic_Codec::decode(Adaptive_Data_Model &M, int forbidden1,
                                  int forbidden2) {
#ifdef _DEBUG
  if (mode != 2)
    AC_Error("decoder not initialized");
...
void Arithmetic_Codec::encode(unsigned data, Adaptive_Data_Model &M,
  ...
  if (forbidden[0] == 0) {
    ...
    if (M.data_symbols - forbidden[0] == 1)
      return;

    forbidden[forbidden[0] + 1] = 0;
    unsigned j = 1, k, sum = 0;
    unsigned data_code = data;
    unsigned count_rest = M.total_count;
    for (int i = 0; i < forbidden[0]; i++)
      count_rest -= M.symbol_count[forbidden[i + 1]];
    ...
    for (k = 0; k < M.data_symbols; k++) {
      ...
      if (k == data)
        data_code = k - (j - 1);
      M.distribution_forbidden[k - (j - 1)] =
    ...
    if (data_code == M.last_symbol - forbidden[0]) {
      x = M.distribution_forbidden[data_code] * (length >> DM__LengthShift);
      base += x;   // update interval
      length -= x; // no product needed
    } else {
...
float Arithmetic_Codec::getBits(unsigned data, Adaptive_Data_Model &M,
                                int *forbidden) {
  if (M.data_symbols == 1 || M.data_symbols - forbidden[0] == 1)
    return 0;

  unsigned count_rest = M.total_count;
  for (int i = 0; i < forbidden[0]; i++)
    count_rest -= M.symbol_count[forbidden[i + 1]];
  float p = float(M.symbol_count[data]) / count_rest;
...
unsigned Arithmetic_Codec::decode(Adaptive_Data_Model &M, int *forbidden) {
  ...
  if (forbidden[0] == 0) {
    ...
    if (M.data_symbols - forbidden[0] == 1) {
      for (int i = 0; i < M.data_symbols - 1; i++) {
        if (forbidden[i + 1] != i)
          return i;
      }
      return M.data_symbols - 1;
    }
    ...
    x = s = 0;
    length >>= DM__LengthShift;
    unsigned m = (n = (M.data_symbols - forbidden[0])) >> 1;
    do {
      unsigned z = length * M.distribution_forbidden[m];
      if (z > value) {
        n = m;
...
int Arithmetic_Codec::getByteNum() { return writeBytes; }
...
void Arithmetic_Codec::start_encoder(void) {
  ...
  mode = 1;
  base = 0; // initialize encoder variables: interval && pointer
...
void Adaptive_Bit_Model::reset(void) {
  ...
  update_cycle = bits_until_update =
      1; // 4;         // start with frequent updates
}
...
int Adaptive_Bit_Model::get_count() { return bit_count; }
...
void Adaptive_Data_Model::printPro(void) {
  for (int i = 0; i < data_symbols - 1; i++) {
    printf("%f ",
           float(int(distribution[i + 1]) - int(distribution[i])) / 32767);
  }
...
void Adaptive_Data_Model::copy(Adaptive_Data_Model &M, int *forbidden) {
...
int Adaptive_Data_Model::get_count() { return total_count; }
...
void Adaptive_Data_Model::update(bool from_encoder) {
  ...
  if ((total_count += update_cycle) > DM__MaxCount) { // DM__MaxCount
    total_count = 0;
